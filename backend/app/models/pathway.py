...
class Pathway(db.Model):
    ...
    items    = db.relationship('PathwayItem', back_populates='pathway',
                               lazy='select', order_by='PathwayItem.order',
    ...
    def to_dict(self, include_items=True):
        data = {
            ...
            'stories_count': len(self.items),
        }
        if include_items:
            data['steps'] = [item.to_dict() for item in self.items]
        return data
...
class PathwayItem(db.Model):
    ...
    def to_dict(self):
        return {
            'id':    self.id,
            'order': self.order,
            'story': self.story.to_step_dict() if self.story else None,
        }
